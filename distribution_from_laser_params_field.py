...
def interpolate_field(field_file='gaus2square.npy', tay13scalefactor=0, nradius=3):
    # field_file <string> path to field data to interpolate
    ...
    # interpolate the real and imaginary parts of the E-field
    myinterp = []
    for t in unique_ts:
        cut = pss[:,1] == t
        myinterp += [[t,np.interp(sf,pss[cut,0],pss[cut,2]),np.interp(sf,pss[cut,0],pss[cut,3])]]
...
def filtered_green_profile(tay13scalefactor=1., filter_bw_fwhm_nm=1., filter_bg_passfraction=0., field_file='gaus2square.npy', plotQ=False):
    ...
    Evst = interpolate_field(field_file=field_file, tay13scalefactor=np.abs(tay13scalefactor))
    Evst[:,0] *= t_sign # reverse time
    t = Evst[:,0]*1e-12; dt = np.abs(t[1]-t[0])# seconds
    p0 = Evst[:,1]**2 + Evst[:,2]**2 # intensity (arb. units)
    E = Evst[:,1] + 1j * Evst[:,2] # field (arb. units)
    ...
    fft = np.fft.fft(E) # fft of the field strength (assumes slowly varying phase)
    ...
    fft_filter = np.sqrt(fft_filter) # filter is defined in intensity, but applied to field
    if plotQ:
        ...
        plt.plot(lambdas, absfft**2, label='input')
        plt.plot(lambdas, (absfft*shiftfilt)**2, label='output'); plt.xlabel('wave length (nm)')
        plt.plot(lambdas, shiftfilt**2, label='filter')
        plt.legend(); plt.show(); plt.close()
    
    p = np.abs(np.fft.ifft(fft * fft_filter))**2
    if plotQ:
        plt.plot(Evst[:,0],p0, label='input')
        plt.plot(Evst[:,0],p, label='output'); plt.xlabel('time (ps)'); 
        plt.legend(); plt.show(); plt.close()
    print('Fraction of power filtered:',np.sum(p)/np.sum(p0) - 1.);
    pvst = Evst[:,:2]
    pvst[:,1] = p
...
def make_beam(npart=int(5e4), tay13scalefactor=1., filter_bw_fwhm_nm=1., filter_bg_passfraction=0., t_origin_ps=0., field_file='gaus2square.npy', sigmax=300e-6, cut_radius_x=450e-6, pr_eV_mean=4.*1240./1030.-2.86, pr_eV_rms=25.7e-3, plotQ=False):
    # tay13scalefactor <float> should be a real number in range [-3.,3.]:
    #   NOTE: the sign of tay13scalefactor flips the origin of time
    #   0 => Gaussian; 1 => flat-top; 2 => triangle; 3 => smoother triangle facing opposite direction
    # filter_bw_fwhm_nm <float> [0.,1e3] is the fwhm width of the band pass filter; 1 should smooth most ripples; 10 should pass everything
    ...
    pvst = filtered_green_profile(tay13scalefactor=tay13scalefactor, filter_bw_fwhm_nm=filter_bw_fwhm_nm, filter_bg_passfraction=filter_bg_passfraction, field_file=field_file, plotQ=plotQ)
